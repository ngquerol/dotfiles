...
--   + rename it to cal
-- modified by Nicolas G. Querol <nicolas.gquerol@gmail.com> (2012), under the
-- same license, and with the following changes:
...
function displayMonth(month,year,weekStart)
    ...
    for x=0,6 do
        -- remove the trailing dot to ensure good alignment
        local weekdays = os.date("%a",os.time{year=2006,month=1,day=x+wkSt})
        weekdays = string.gsub(weekdays, "%p", " " )
        lines = lines .. "<span bgcolor='#303030'>" .. weekdays .. "</span>"
    end

    lines = lines .. "\n" .. os.date(" <span bgcolor='#303030'>%V</span>",os.time{year=year,month=month,day=1})
    ...
    while writeLine < (stDay + 1) do
        ...
        for d=1,mthDays do
                ...
                if writeLine == 8 then
                        writeLine = 1
                        lines = lines .. "\n" .. os.date(" <span bgcolor='#303030'>%V</span>",t)
                end
    ...
    if not tooltip then
        tooltip = awful.tooltip({})
                function tooltip:update()
                        ...
                        state = {month, year}
                        tooltip:set_text(string.format('<span font_desc="terminusmodx">%s</span>', displayMonth(month, year, 2)))
                end
                tooltip:update()
    end
...
function switchMonth(delta)
    ...
    local text = string.format('<span font_desc="terminusmodx">%s</span>', displayMonth(state[1], state[2], 2))
